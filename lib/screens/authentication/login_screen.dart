...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> signInWithEmailAndPassword() async {
    ...
    User? user = await AuthenticationController().signInWithEmailAndPassword(
        context,
  ...
  Future<void> sendPasswordResetLink() async {
    ...
    bool isPasswordResetemailSent = await AuthenticationController()
        .sendPasswordResetMail(context, emailController.text);
  ...
  Future<void> onSuccess(User user,
      {bool createAccountIfNotExist = false}) async {
  ...
  Widget getLoginText2() {
    return Container(
      margin: EdgeInsets.only(
          left: MySize.size48!, right: MySize.size48!, top: MySize.size40!),
      child: Text(
  ...
  Widget getForgotPasswordLinkText() {
    return Container(
      margin:
          EdgeInsets.only(left: MySize.size48!, right: MySize.size48!, top: 10),
      child: InkWell(
        onTap: () async {
          if (emailController.text.trim().isNotEmpty) {
            sendPasswordResetLink();
          } else {
            Snakbar().show_error_snakbar(context, "Enter Email");
          }
        },
        child: Text(
          "Forgot Password?",
  ...
  Widget getLoginWithGoogleButton() {
    return InkWell(
      ...
      child: Container(
        margin: EdgeInsets.only(
            left: MySize.size24!, right: MySize.size24!, top: MySize.size36!),
        decoration: BoxDecoration(
          color: Styles.primaryColor,
          borderRadius: BorderRadius.circular(MySize.size10!),
          boxShadow: [
